...
_DEFAULTS = {
    ...
    "loaded_config": {}, # Added for config Import/Export
}
...
# ============================================================================
# DYNAMIC DATASET INTROSPECTION & EDA SCRIPT
# ============================================================================
@st.cache_data(show_spinner=False, ttl=3600)
def fetch_dataset_preview(dataset_key):
    """Fetches up to 500 rows of a dataset for Schema and EDA profiling."""
...
# ============================================================================
# CONFIGURATION STATE MANAGER (YAML Import/Export)
# ============================================================================
def get_cfg(keys, default):
    """Traverses the loaded_config dictionary securely."""
        ...
        if df_preview is not None and not df_preview.empty:
            ...
            with c_info:
                ...
                info_df = pd.DataFrame({
                    ...
                    "Missing Values": df_preview.isna().sum(),
                    "Missing %": (df_preview.isna().sum() / len(df_preview) * 100).round(2)
                })
...
def render_configuration_section():
    ...
    if schema_cols:
        ...
        context_columns_list = st.multiselect(
            ...
            options=schema_cols,
            default=[c for c in loaded_ctx if c in schema_cols],
    ...
    with tab_training:
        ...
        epochs = st.number_input("VAE Epochs", min_value=1, value=int(get_cfg(("vae", "epochs"), 1000)), step=100)
        diffusion_epochs = st.number_input("Diffusion Epochs", min_value=1, value=int(get_cfg(("diffusion", "epochs"), 2000)), step=100)
        vae_lr = st.number_input("VAE Learning Rate", min_value=0.00001, value=float(get_cfg(("vae", "learning_rate"), 0.001)), step=0.0001, format="%.5f")
        diff_lr = st.number_input("Diffusion Learning Rate", min_value=0.00001, value=float(get_cfg(("diffusion", "learning_rate"), 0.001)), step=0.0001, format="%.5f")
        batch_size = st.number_input("Batch Size", min_value=1, value=int(get_cfg(("vae", "batch_size"), 256)), step=32)

    # ---------------- VAE ----------------
    with tab_vae:
        st.subheader("Manifold Architecture")
        
        lm_options = ["Adaptive", "Custom"]
        loaded_lm = str(get_cfg(("vae", "latent", "mode"), "adaptive")).capitalize()
        lm_idx = lm_options.index(loaded_lm) if loaded_lm in lm_options else 1
        latent_mode = st.radio("Latent Dimension Strategy", lm_options, index=lm_idx, horizontal=True)
        ...
        if latent_mode == "Adaptive":
            ...
            latent_dim = st.number_input("Latent Dimension", min_value=2, value=int(get_cfg(("vae", "latent", "latent_dim"), 256)), step=2)
            encoder_dims = st.text_input("Encoder Dimensions", str(get_cfg(("vae", "latent", "encoder_dims"), [2048, 1024, 512])))
            if st.checkbox("Auto-reflect decoder", value=True):
                ...
                decoder_dims = st.text_input("Decoder Dimensions", str(get_cfg(("vae", "latent", "decoder_dims"), [512, 1024, 2048])))

        st.markdown("**MMD Kernel Distance**")
        mmd_opts = ["Constant", "Linear", "Cosine", "Adaptive"]
        loaded_mmd = str(get_cfg(("vae", "mmd", "mode"), "constant")).capitalize()
        mmd_idx = mmd_opts.index(loaded_mmd) if loaded_mmd in mmd_opts else 0
        mmd_mode = st.radio("MMD Schedule", mmd_opts, index=mmd_idx, horizontal=True)
        mmd_config = {"mode": mmd_mode.lower()}

        if mmd_mode == "Constant":
            mmd_config["weight"] = st.number_input("MMD Base Weight", min_value=0.0, value=float(get_cfg(("vae", "mmd", "weight"), 500.0)), step=10.0)
        elif mmd_mode == "Linear":
            c1, c2 = st.columns(2)
            mmd_config["start_weight"] = c1.number_input("Start Weight", value=float(get_cfg(("vae", "mmd", "start_weight"), 0.0)), step=10.0)
            mmd_config["end_weight"] = c2.number_input("End Weight", value=float(get_cfg(("vae", "mmd", "end_weight"), 500.0)), step=10.0)
            mmd_config["warmup_epochs"] = st.number_input("Warmup Epochs", min_value=1, value=int(get_cfg(("vae", "mmd", "warmup_epochs"), 250)), step=50)
        elif mmd_mode == "Cosine":
            c1, c2 = st.columns(2)
            mmd_config["min_weight"] = c1.number_input("Min Weight", value=float(get_cfg(("vae", "mmd", "min_weight"), 0.0)), step=5.0)
            mmd_config["max_weight"] = c2.number_input("Max Weight", value=float(get_cfg(("vae", "mmd", "max_weight"), 500.0)), step=10.0)
            mmd_config["period"] = st.number_input("Cosine Period (epochs)", min_value=1, value=int(get_cfg(("vae", "mmd", "period"), 1000)), step=100)
        else:
            mmd_config["target_ratio"] = st.number_input("Target Reconstruct Ratio", min_value=0.01, value=float(get_cfg(("vae", "mmd", "target_ratio"), 0.20)), step=0.01, format="%.2f")
            mmd_config["momentum"] = st.number_input("Momentum Term", min_value=0.0, max_value=0.999, value=float(get_cfg(("vae", "mmd", "momentum"), 0.95)), step=0.01, format="%.3f")

        st.divider()
        c3, c4 = st.columns(2)
        
        act_opts = ["SiLU", "ReLU", "GELU", "LeakyReLU"]
        loaded_act = get_cfg(("vae", "activation"), "LeakyReLU")
        act_idx = act_opts.index(loaded_act) if loaded_act in act_opts else 3
        activation = c3.selectbox("Activation Function", act_opts, index=act_idx)
        
        dropout = c4.number_input("Dropout Rate", min_value=0.0, max_value=1.0, value=float(get_cfg(("vae", "dropout"), 0.0)), step=0.05)
        use_layernorm = st.checkbox("Enable LayerNorm", value=bool(get_cfg(("vae", "layernorm"), True)))

    # ---------------- DIFFUSION ----------------
    with tab_diffusion:
        st.subheader("Denoising MLP")
        
        diff_opts = ["Standard", "Residual", "TabDiff", "Custom"]
        loaded_diff = str(get_cfg(("diffusion", "architecture", "type"), "standard")).capitalize()
        diff_idx = diff_opts.index(loaded_diff) if loaded_diff in diff_opts else 0
        diffusion_architecture = st.selectbox("Network Topology", diff_opts, index=diff_idx)
        
        num_diffusion_steps = st.number_input("Markov Timesteps ($T$)", min_value=1, value=int(get_cfg(("diffusion", "num_diffusion_steps"), 2000)), step=100)

        # Dynamic hidden layer default based on arch selection OR loaded config
        loaded_h_dims = str(get_cfg(("diffusion", "architecture", "hidden_dims"), "[512, 512, 256]"))
        ...
        else: diffusion_hidden_dims = st.text_input("Hidden Layers", loaded_h_dims)

        if diffusion_architecture != "Custom":
            st.text_input("Hidden Layers (Locked)", value=diffusion_hidden_dims, disabled=True)
            
        diffusion_dropout = st.number_input("Denoising Dropout", min_value=0.0, max_value=1.0, value=float(get_cfg(("diffusion", "architecture", "dropout"), 0.1)), step=0.05)

        st.markdown("**Noise Beta Schedule**")
        sch_opts = ["Linear", "Cosine", "Quadratic", "Sigmoid", "Learnable"]
        loaded_sch = str(get_cfg(("diffusion", "noise_schedule", "type"), "linear")).capitalize()
        sch_idx = sch_opts.index(loaded_sch) if loaded_sch in sch_opts else 0
        noise_schedule = st.selectbox("Variance Scheduler", sch_opts, index=sch_idx)
        noise_config = {"type": noise_schedule.lower()}

        if noise_schedule != "Learnable":
            c5, c6 = st.columns(2)
            noise_config["beta_start"] = c5.number_input("$\\beta_{start}$", min_value=0.0, value=float(get_cfg(("diffusion", "noise_schedule", "beta_start"), 0.0001)), format="%.5f")
            noise_config["beta_end"] = c6.number_input("$\\beta_{end}$", min_value=0.0, value=float(get_cfg(("diffusion", "noise_schedule", "beta_end"), 0.0200)), step=0.005, format="%.5f")
        else:
        ...
        t_emb_opts = ["Sinusoidal", "Learned", "Fourier"]
        loaded_t_emb = str(get_cfg(("diffusion", "time_conditioning", "embedding"), "sinusoidal")).capitalize()
        t_emb_idx = t_emb_opts.index(loaded_t_emb) if loaded_t_emb in t_emb_opts else 0
        time_embedding_type = c7.selectbox("Temporal Embedding", t_emb_opts, index=t_emb_idx)
        
        t_cond_opts = ["Addition", "FiLM", "Concatenation"]
        loaded_t_cond = str(get_cfg(("diffusion", "time_conditioning", "method"), "film")).capitalize()
        # Edge case fix for FiLM capitalization 
        loaded_t_cond = "FiLM" if loaded_t_cond == "Film" else loaded_t_cond
        t_cond_idx = t_cond_opts.index(loaded_t_cond) if loaded_t_cond in t_cond_opts else 1
        time_conditioning = c8.selectbox("Modulation Approach", t_cond_opts, index=t_cond_idx)
        
        time_embedding_dim = st.number_input("Temporal Projection Dim", min_value=8, value=int(get_cfg(("diffusion", "time_conditioning", "embedding_dim"), 128)), step=8)
    ...
    # Build Config Dict for export or running
    config = {
    ...
    with right:
        ...
        st.download_button("📤 Export Config", data=yaml_str, file_name=f"synthcity_{selected_dataset}_cfg.yaml", mime="text/yaml", use_container_width=True)

    if run_model:
        # 1. Save Config for orchestrator
