...
# ============================================================================
# STYLES (Professional Dark Theme with Centered Sidebar Titles)
# ============================================================================
st.markdown(
    """
    <style>
    ...
    /* Centered Header Text in Sidebar */
    ...
    .sc-divider-label::before { margin-right: 16px; }
    .sc-divider-label::after { margin-left: 16px; }
    </style>
    """,
...
with st.sidebar:
    ...
    st.markdown(
        ...
        <div class="sidebar-header-center">
            ...
            <div style="color:#94a3b8;font-size:11px;line-height:1.3;margin-top:3px;">
                Tabular Generative Engine & Diagnostics
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    # Centered Section Header: Navigation
    ...
    st.selectbox("Hardware Accelerator", ["Auto-detect", "CPU", "CUDA (GPU)", "MPS (Apple Silicon)"])

    # Centered Section Header: Resource Monitors
    st.markdown('<div class="sidebar-section-title">Resource Monitors</div>', unsafe_allow_html=True)
...
def render_new_run_page():
    ...
    st.markdown(f'<div class="sc-section-title">Model Architecture & Dataset Selection: (Choose/Upload){ds_tick}</div>', unsafe_allow_html=True)
    
    col_model, _ = st.columns([1, 2])
    with col_model:
        model_options = {
            "Latent-Diffusion 2.2": "Latent-Diffusion 2.2 (Latent Diffusion)",
            "WGAN-GP 2.1": "WGAN-GP 2.1 (Wasserstein GAN with Gradient Penalty)",
    ...
    with st.expander(" Pre-Training Data Profiler (EDA)"):
...
def render_configuration_section():
    ...
    st.markdown(f'<div class="sc-section-title">Contextual Injection Constraints(optional) &middot; {label}{ctx_tick}</div>', unsafe_allow_html=True)
    ...
    st.markdown(f'<div class="sc-section-title">Hyperparameter Workbench{wb_tick}</div>', unsafe_allow_html=True)
    st.markdown("<p style='color:#94a3b8;font-size:13.5px;margin-bottom:14px;'>Upload a YAML configuration to auto-fill the environment, OR manually tune the engine parameters below.</p>", unsafe_allow_html=True)
    ...
    if st.session_state.loaded_config:
        ...
        if st.button("Clear Configuration (Manual Override)"):
        ...
        tab_training, tab_vae, tab_diffusion = st.tabs(["Training Parameters", "InfoVAE Setup", "Latent Diffusion Network"])
    ...
    if not context_columns_list:
        ...
        st.markdown("<div style='color:#94a3b8;font-size:13.5px;margin-bottom:12px;'>Declare structural values to steer generation across the synthesized batch.</div>", unsafe_allow_html=True)
        blueprint_values = {}
    ...
    if run_model:
        ...
        with st.spinner("Compiling graphs and tracking to MLflow..."):
            try:
                ...
                st.error("Backend script missing: alt_main.py")
...
# ============================================================================
# PAGE: DOCUMENTATION (COMPREHENSIVE KNOWLEDGE BASE)
# ============================================================================
def render_documentation_page():
    st.markdown(
        ...
        <div class="sc-banner">
            ...
            <p>A novice-friendly reference guide covering model mechanics, statistical audit metrics, and latent health diagnostics.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    
    with st.expander("🧠 Generative Frameworks Explained", expanded=True):
        st.markdown("""
        **1. Latent-Diffusion 2.2**
        *   **How it works:** Instead of training a complex model directly on raw tabular columns, a Variational Autoencoder (VAE) compresses the tabular data into a smooth continuous latent space. A diffusion model then learns to denoise random Gaussian vectors within this latent space.
        *   **Why use it:** Highly effective for mixed numerical and categorical tables; avoids direct feature memorization.

        **2. WGAN-GP 2.1 (Wasserstein GAN with Gradient Penalty)**
        *   **How it works:** Replaces standard Generative Adversarial Networks (GANs) that use Jensen-Shannon divergence with Earth Mover's (Wasserstein-1) Distance. It enforces Lipschitz continuity via a penalty on the gradient norm of the Critic.
        *   **Why use it:** Standard GANs suffer from severe *mode collapse* (generating only one type of row repeatedly). WGAN-GP stabilizes training and yields smooth convergence curves without collapsing.

        **3. Vanilla VAE 1.3 (Variational Autoencoder)**
        *   **How it works:** Maps input records into a Gaussian distribution $q_\phi(z|x)$ parametrized by mean $\mu$ and variance $\sigma^2$, penalized by Kullback-Leibler (KL) divergence against a standard normal prior $p(z) \sim N(0, I)$.
        *   **Why use it:** Extremely fast to train, though it can generate overly smoothed distributions for sharp tabular boundaries.

        **4. InfoVAE-MMD 1.5**
        *   **How it works:** Addresses "information preference" issues in standard VAEs where the latent space is ignored. It uses Maximum Mean Discrepancy (MMD) as a non-parametric kernel distance to align the aggregate posterior distribution with the prior.
        *   **Why use it:** Maximizes mutual information between latents and features while keeping the latent space well-behaved.
        """)
        
    with st.expander("📊 Evaluation Metrics Glossary (Utility & Fidelity)", expanded=True):
        st.markdown("""
        When auditing synthetic data, SynthLab evaluates several key statistical metrics:

        *   **Shape Error:** Evaluates individual column distributions (1D marginals). It measures the histogram difference between real and synthetic columns.
            $$\\text{Shape Error} \to 0 \\text{ indicates perfect 1D distribution match.}$$
        *   **Trend Error:** Evaluates feature-to-feature interactions (2D correlation structure). It measures how closely pairwise column correlations match between the real dataset and the synthetic dataset.
        *   **Alpha-Precision ($\\alpha$):** Measures **Fidelity** (Sample Quality). It checks whether synthetic records fall within the mathematical boundary (support) of the real data. An Alpha-Precision of $1.0$ means zero synthetic records are unrealistically out of bounds.
        *   **Beta-Recall ($\\beta$):** Measures **Diversity** (Data Coverage). It checks if synthetic data covers the entire distribution space of the real data. High recall guarantees rare edge cases in real data are represented in synthetic data.
        *   **Normalized MSE:** Reconstruction Mean Squared Error divided by target variance:
            $$\\text{Normalized MSE} = \\frac{\\text{MSE}}{\\text{Var}(Y)}$$
            A value $<1.0$ means decoding errors are significantly smaller than the natural variance of the real data.
        """)

    with st.expander("🧬 Interpreting the Latent Diagnostic Report", expanded=True):
        st.markdown("""
        SynthLab inspects the internal geometry of the encoder $q(z|x)$ and decoder $p(x|z)$ using `diagnostics.latent` . Below is a guide to interpreting these metrics:

        #### 1. Encoder Distribution & Posterior Standard Deviation
        *   **Posterior Std ($\mu_{\sigma}$):** Measures the standard deviation of encoder outputs $\mu$ 
        *   **Healthy Range:** $0.75 \le \sigma_{post} \le 1.25$ .
        *   **Meaning:** If $\sigma_{post} \ll 0.75$, the posterior is collapsing toward a single point. If $\sigma_{post} \gg 1.25$, the encoder is drifting away from the prior.

        #### 2. Latent Utilization & Dead Dimensions
        *   **Active Dimensions:** Count of latent dimensions with variance $std(\mu) > 0.10$ .
        *   **Dead Dimensions:** Dimensions with KL divergence $KL < 0.01$.
        *   **Diagnosis:** If active dimensions ratio $< 80\%$ or dead dimensions ratio $> 20\%$, the model is suffering from **Posterior Collapse** (ignoring latent dimensions).

        #### 3. Effective Rank & Latent Diversity
        *   **Effective Rank:** Number of non-zero eigenvalues ($>10^{-3}$) of the latent covariance matrix .
        *   **Meaning:** Measures true dimensionality. If latent dimension is $256$ but effective rank is $8$, the latent space is squashed onto a small subspace.

        #### 4. Condition Number & Anisotropy
        *   **Condition Number:** Ratio of the largest to smallest eigenvalue of the covariance matrix:
            $$\\text{Condition Number} = \\frac{\\lambda_{\\max}}{\\lambda_{\\min} + 10^{-8}}$$ 
        *   **Diagnosis:** Values $>10^4$ indicate extreme anisotropy (the latent space is severely stretched along one axis and compressed along another).

        #### 5. Latent Independence & Max Correlation
        *   **Max Abs Correlation:** Maximum off-diagonal entry of the latent correlation matrix .
        *   **Healthy Threshold:** $< 0.60$ . High correlation means latent dimensions are redundant and entangled.

        #### 6. Decoder Sensitivity & Smoothness
        *   **Decoder Sensitivity:** Measures output changes when latents are perturbed by $\epsilon \sim N(0, \delta^2 I)$ :
            $$\\text{Sensitivity} = |\\text{Decoder}(z + \\epsilon) - \\text{Decoder}(z)|$$ 
        *   **Interpolation Change:** Evaluates smoothness when stepping along a straight path $z_a \to z_b$ in latent space. Smooth changes ensure interpolations yield coherent synthetic samples .
        """)
